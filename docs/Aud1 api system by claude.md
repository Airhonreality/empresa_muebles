Auditoría: Agnostic System — Subsistema de Exposición           
  ...
  --
  auditoria 2 GEMINI:
  # 🛰️ Master Alignment Plan: Indra v2027

Este plan detalla las acciones necesarias para elevar el Agnostic System a un estado de **Gracia Técnica**, blindando el contrato entre el ADN y la Materia.

## 🎯 Objetivo
Transformar el framework de un "sistema de convenciones implícitas" a una "librería de contratos explícitos", mejorando la resiliencia y la navegabilidad para agentes de IA y desarrolladores humanos.

---

## 🛠️ Fase 1: Blindaje del Contrato (AI-Navigability)
*Prioridad: Alta | Riesgo: Nulo*

- [ ] **Creación de `agnostic.d.ts` Canónico**:
    - Definir uniones discriminadas para todas las acciones del `bridge.ts` (`WRITE`, `DELETE`, `OPEN_OVERLAY`, `NAVIGATE`).
    - Tipar el objeto `api.ui` para que el IDE (y yo) sepamos qué props acepta cada componente de shadcn/ui.
- [ ] **Refactor de `bridge.ts`**:
    - Eliminar los `any` restantes.
    - Implementar retornos de tipo `Promise<AgnosticResult<T>>` para manejar errores de red de forma elegante.

## 🔒 Fase 2: Blindaje de Materia (Persistence Hardening)
*Prioridad: Alta | Riesgo: Medio*

- [ ] **Validación Zod en `vault.ts`**:
    - Antes de guardar en Supabase o Local, el sistema debe validar el payload contra el esquema. Si no cumple, el Intent es rechazado antes de ensuciar la DB.
- [ ] **Optimización de Estrategias**:
    - Implementar el patrón **Singleton** en `getStrategy.ts` para evitar lecturas de disco redundantes por cada petición.
    - Añadir una capa de caché (TTL 5 min) para los esquemas del ADN.

## 🎨 Fase 3: Proyección de Lujo (UI Sovereignty)
*Prioridad: Media | Riesgo: Bajo*

- [ ] **Integración Nativa de shadcn/ui**:
    - Completar el `OverlayOrchestrator` para soportar `Popover` y `Select` de forma agnóstica.
    - Estandarizar el uso de **CVA (Class Variance Authority)** en los bloques soberanos para manejar estados visuales (Loading, Error, Edit).

## 🚫 Exclusiones (Protección de Soberanía)
- **NO instalar Clerk o NextAuth**: Mantendremos el sistema de identidad propio para asegurar que el satélite sea autoportante y no dependa de suscripciones externas.
- **NO implementar paginación compleja aún**: Priorizaremos la agilidad del Cotizador antes que la escalabilidad de grandes volúmenes de datos.

---

## 🚦 Reglas de Ejecución
1. **Unidades Relativas Siempre**: Ni un solo `px` será introducido.
2. **Tailwind para todo**: Prohibido el CSS inline o inyectado.
3. **Comentarios de Razón de Ser**: Todo cambio debe explicar el *porqué* arquitectónico.
