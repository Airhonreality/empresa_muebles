...
| **F10** | Prototipo con mocks (ABIERTA — bloque 1 en curso) | **Plan aprobado** — `tecnico/plan_f10_migracion.md` |

**✅ CORTE DE BUCLE DE ARNÉS + tsc EN VERDE (2026-08-12, sesión agente):** creado `arnes/TAREAS_DIFERIDAS.md` (tareas postergadas = NO bloqueantes para migración/preview) — corta el bucle de fases que se reabrían solas. Confirmado que los lotes `lib/data/` ya estaban resueltos en el working tree. Corregidos errores tsc: `SHOP_CATEGORIAS` (valor, no tipo), `testimonios` en `DataStore`/`drizzle-impl.ts`, typo `colecciones`, 2 typings PDF. Resultado: **`tsc --noEmit` exit 0**. Nada commiteado. Detalle en `estado.md` (entrada 2026-08-12).

**Próxima acción permitida (actualizado 2026-08-12):**
> Los 2 lotes de `lib/data/` (verificador_id en P-12, padreLinaje/orden_armado en módulo/espacio, métodos porProyecto de recepcionesMaterial/registrosGateCaja) **YA ESTÁN RESUELTOS en el working tree** — verificado en `arnes/TAREAS_DIFERIDAS.md` §3. Con eso, el prerequisito de datos para F10 está satisfecho. **DP-01/DP-02/DP-03 ya están resueltas o diferidas intencionalmente** (ver `TAREAS_DIFERIDAS.md`). Camino a cierre: (1) confirmar tsc/eslint/build limpios del working tree, (2) cerrar F10 (`plan_f10_migracion.md` §4: actualizar `REGISTRO_DE_ENTIDADES.md`, registro de pantallas "APROBADO-PRE-CODIGO"), (3) push a `dev` → preview de Vercel, (4) checkpoint del Supervisor para merge `dev`→`main`.
...
**✅ BATCH DE AUDITORÍA DE JAVIER + BARRIDO DE CUMPLIMIENTO DISEÑO↔CÓDIGO, EJECUTADO (2026-08-10, D-11 a D-17):** Javier trajo 4 hallazgos puntuales en vivo (ergonomía + IVA del Cotizador P-04, campos hardcodeados del modal de Contrato, Propuesta pública F-08 incompleta respecto a su propia destilación) más el pedido explícito de verificar si otras pantallas se habían codificado sin cotejar contra su `disenio_PXX.md` aprobado — sospecha de "diseño superficial". Se corrió un barrido de auditoría (2 rondas, mismo método: agentes Explore comparando reglas de negocio/criterios de aceptación del diseño contra el código real) sobre las 22 pantallas que nunca habían tenido este nivel de re-auditoría desde que se construyeron. **Ronda 1 (15 pantallas, 11 con hallazgos):** P-18 (`adelantadaPor` nunca se asignaba), P-01 (fórmula de total incompleta + sin validación server-side de transición), P-02 (sin validar cliente/dirección obligatorios), P-03 (faltaban botones "Ver PDF"/"Generar Contrato"), P-21 Caja (pago y obligación desincronizados, sin FK), P-23 (`marcarPagada` sin validación real), P-27 (anexar a catálogo no prellenaba nada), F-02 Tienda Web (fuga de `precioDirecto`/`stockActual`/`proveedorId` + sin filtro de visibilidad), F-07 Portal Cliente (comentario que afirmaba un filtro de privacidad `visible_al_cliente` inexistente — passthrough real). Limpias: P-17, P-19, P-24, P-26. **Ronda 2 (Bloque 2 cronograma/gates + módulo/espacio + Home, 7+2 con hallazgos):** P-06 (hub solo pintaba 2 de 5 gates), P-07 (sin auto-save), P-08 (el flujo de aprobar/rechazar esquema era inalcanzable desde la UI — ningún botón movía el estado a `para_revision`), P-09 (botón "Decisión manual" ausente pese a que el método del store ya existía), P-10 (`escaladoA` nunca se pasaba al escalar una novedad), **P-11 crítico** (el botón "Confirmar check" en su camino normal nunca hacía nada — comparaba contra un campo `desenlaceFinal` que empieza en `null`, y el override inferío el desenlace de un `.includes('extremo')` en vez de una elección explícita del verificador, violando DP3 del mini-diamante), P-12 (sin componente para designar `verificador_id`). Módulo/espacio: **violación de la premisa central del propio diseño** — los gates se evalúan por proyecto completo, no por `modulos.id`, exactamente lo que el diseño advertía que rompía el despacho parcial; más `padreLinaje` faltante y un valor faltante en `TipoModuloArtefacto`. Home (F-01): metadata/SEO desactualizada sin JSON-LD pese a que el propio checklist del doc de contenido lo daba por resuelto, y la grilla "Espacios que creamos" sin ningún link. Todo registrado fila por fila en `tecnico/backlog_auditoria_pantallas.md` (D-11 a D-17). **Ejecución:** sub-agentes Haiku autónomos, mismo modo operativo que POC-20 (sin commit, verificación mecánica obligatoria) — la mayoría de los hallazgos puntuales quedaron corregidos y verificados (`tsc`/`eslint` 0 errores en el árbol completo tras cada lote). **Pendiente real, sin ejecutar:** 2 lotes chicos que tocan `lib/data/` compartido (designar `verificador_id` en P-12; agregar `padreLinaje`/`orden_armado` a módulo/espacio + los métodos `porProyecto()` que le faltan a `recepcionesMaterial`/`registrosGateCaja`, documentados como gap por el propio lote de P-06 en vez de tocarlos sin permiso); y la pieza grande diferida de módulo/espacio (gates por nodo, no por proyecto) — deliberadamente sin tocar, es demasiado grande para un fix ciego y necesita alcance claro de Javier primero, mismo criterio que P-20/P-22/D-08b. **Nada de esto está commiteado** — se suma a la cola de revisión en vivo de Javier junto con D-01→D-10.

**✅ AUDITORÍA EN VIVO DE JAVIER — 7 DIAMANTES + 3 HALLAZGOS DE CÓDIGO, EJECUCIÓN AUTÓNOMA (2026-08-10, POC-20):** Javier auditó el prototipo en el navegador (2 rondas) y compiló los hallazgos en `tecnico/backlog_auditoria_pantallas.md` antes de ejecutar nada. Cambio operativo: los sub-diamantes se ejecutan **sin aprobación por paso**, con mecanismos de seguridad (se detienen antes de `git commit`, no tocan `lib/data/`/`lib/auth/`, verificación mecánica obligatoria) — decidido vía `EnterPlanMode`/`ExitPlanMode` formal. 7 diamantes ejecutados: web pública (D-01, nuevo lenguaje editorial vs. el del ERP), nav ERP (D-02, sidebar de 5 secciones), nav público (D-03, resuelto dentro de D-01), Compras (D-04, lista de ítems vía schema aprobado + ítems "a pedido"), Herramientas (D-05, máquina de estados corregida — reconocida como falla de mi propio diseño original de P-15), Finanzas (D-06, dashboard de KPIs), Taller (D-07, interacción con filtros + modal). D-08 (perfil de Persona/Proveedor) se dividió: D-08a (vista gerente) ejecutada, D-08b (autogestión) diferida — requiere decisión sobre login interno, no existe hoy. Más C-01 (bug real corregido: `ordenesCompra.crear()` no generaba obligación, incoherencia medible Obligaciones/Caja), C-02 (foto en Equipo), C-03 (Cronograma agregado al nav). **Incidente de infraestructura:** el aislamiento por worktree (`isolation: "worktree"`) falló sistemáticamente — Git no permite la misma rama en dos worktrees, así que los agentes se crearon ramificados desde `main` sin código de V3; 5 de 6 lo detectaron solos y pidieron permiso en vez de forzar un checkout (cero daño al árbol real). Relanzados sin aislamiento, cerraron limpio. Hallazgo de consolidación no detectable por herramientas automáticas: typo `text-text-display-publico` (Tailwind ignora silenciosamente clases inválidas) en 4 páginas nuevas, corregido. Detalle completo: `tecnico/registro_hallazgos_poc4.md` POC-20. Verificado: tsc/eslint limpios, 92/92 tests, build 40/40 rutas. Nada commiteado — a la espera de revisión en vivo.

**✅ DIAMANTE F4 (Compras) + CIERRE DE F7 (2026-08-10, POC-19):** 5 pantallas — P-13 (orden de compra), P-14 (recepción de material, E-21), P-15 (herramientas, E-45), P-24 (pedidos web, E-44), P-26 (documentación del proyecto, E-41). Javier detectó que F4 nunca había pasado por diseño ni código (hallazgo de POC-18) y ordenó abrir el diamante — 5 `disenio_PXX.md` completos en `arnes/lineas/ola7/pantallas/` **antes** de cualquier código, corrigiendo el error de progreso de POC-18. Se confirmó que P-25 (histórico) ya estaba construido como P-20 (garantía) — sin trabajo nuevo ahí. Decisión de alojador de documentos (P-26, bloqueada desde 2026-08-03) resuelta: R2 para imágenes, Google Drive Desktop local sin integración API para SketchUp/SDK. Capa de datos previa (4 entidades + `lib/modules/f4/gates.ts`) construida por Claude directo antes de repartir pantallas. **Fricción de tooling nueva:** 2 de 6 corridas `opencode` (`deepseek-v4-flash-free`, `ling-3.0-tiny-free`) terminaron con exit 0 sin escribir ningún archivo — silenciosas, sin error. Detectado verificando la existencia del archivo, no el exit code. Relanzadas con otro modelo, cerraron limpio. Detalle completo: `tecnico/registro_hallazgos_poc4.md` POC-19. Verificado: tsc/eslint limpios, 85/85 tests (55 mock-store + 15 f3 + 3 f4 + 12 f4f5f6), build 30/30 rutas. Nada de esto está commiteado.
...
- Árbol recursivo `modulos` (padre_id self, `padre_linaje` para rastreo E-54). **Resuelve el despacho parcial** (gates por nodo, no por proyecto) y el caso de tercero pendiente no bloqueante.
 - Tabla auxiliar `modulos_artefactos` (imagen/plano de armado/3D) con `fuente` = heredado_catálogo / dedicado_proyecto (sin duplicar binario). **Nota (2026-08-12):** El tipo `orden_armado` fue consolidado en `plano_armado` (el documento/plano real).
...
**⚠️ REGISTRO HISTÓRICO, SUPERADO (nota agregada 2026-08-10).** Esta tabla congela el estado del 2026-08-05, antes de que P-02/P-03/F-01/F-02/F-03/F-08 se construyeran y pasaran por su propio ciclo de auditoría — hoy da información incorrecta si se lee como estado actual (ej. marca P-02/P-03 "pendiente" cuando ya están construidas y auditadas). **La fuente viva del estado real de cada pantalla es `tecnico/backlog_auditoria_pantallas.md`** (filas D-01 a D-17) — no esta tabla. Se deja sin borrar, como registro de lo que se sabía en ese momento (mismo criterio de Living Documentation que el resto del arnés: nunca se borra para ocultar, se marca obsoleto).
...
>
> **⚛️ SUPERADO por decisión axiomática (2026-09-10).** La decisión "sin snapshot, siempre en vivo" fue correcta en su momento (evitó el bug del legacy con la información disponible), pero la necesidad de negocio cambió: Javier pidió control de versiones real sobre lo que ve el cliente (botón publicar/nueva versión, timestamp humano, tabs de histórico). El nuevo mecanismo (`propuestas_versiones`, insert-only, snapshot JSONB completo por versión — `arnes/lineas/ola7/tecnico/decision_axiomatica_2026-09-10_header_entidad_y_versionado_propuesta.md`, `t-156`) cierra la clase de bug que este registro evitaba (desincronización *dentro* de un mismo snapshot mutable) sin renunciar al versionado. `disenio_F08_propuesta_publica.md` R1 queda como la regla vigente.
