...
- **Realidad encontrada:** `app/erp/finanzas/page.tsx`, `app/erp/finanzas/parametros/page.tsx` y `lib/modules/finanzas/` (fórmulas `calcularCostoEnsamblaje`, `PARAMETROS_DEFAULT`) existen en working tree sin `plan_f11.md` aprobado ni tarea en ledger — son de F11 (panel de parametrización), fuera del alcance del bloque 1 de `plan_f10_migracion.md` §2.
...
- **Artefactos modificados:**
  ...
  - `arnes/lineas/ola7/tecnico/plan_f10_migracion.md` — P-27 añadida al bloque B1
...
- **Artefactos modificados:**
  ...
  - `plan_f10_migracion.md` §1.2 — referencia al contrato M-07 para pantallas nuevas de B2 en adelante.
...
- **Descartado:** la propuesta de rutas `/api/publico/*`/`/api/erp/*` con Drizzle real de uno de los reportes — viola `plan_f10_migracion.md:115` (ninguna conexión real durante F10) y `contracts.ts:3` (las pantallas nunca importan `lib/db/`). `lib/db/client.ts` es una conexión REAL a dev-local de Neon, no un stub — de haberse construido tal cual, habría tocado base de datos real. Archivado para F10-E.
...
## POC-20 — Auditoría en vivo de Javier (proceso continuo, abierto)

**Severidad:** alto · **Origen:** Javier está auditando el prototipo pantalla por pantalla en el navegador, en múltiples rondas seguidas. **Fuente única de verdad de los hallazgos y su estado:** `arnes/lineas/ola7/tecnico/backlog_auditoria_pantallas.md` — no se duplica acá (ver nota de 2026-08-10 abajo). Esta entrada registra solo lo que ese documento no cubre: decisiones de proceso/tooling y snapshots de verificación mecánica por ronda.

- **Cambio operativo (ronda 1):** Javier pidió que los diamantes del backlog se ejecuten **sin aprobación por paso**, con mecanismos de seguridad (se detienen antes de `git commit`; no tocan `lib/data/`/`lib/auth/`; verificación mecánica obligatoria) — decidido vía modo Plan formal (`EnterPlanMode`/`ExitPlanMode`).
- **Cambio de tooling dentro de la sesión:** Javier pidió usar Haiku (Agent tool) en vez de `opencode`, que estaba ocupado — instrucción puntual de esta sesión, no un cambio de regla permanente.
- **Incidente de infraestructura (ronda 1):** los primeros 6 lotes se lanzaron con `isolation: "worktree"` como mecanismo de seguridad extra. Falló sistemáticamente — Git no permite la misma rama (`dev`) en dos worktrees a la vez, así que cada worktree aislado se creó ramificado desde `main` (`fbe9bdd`), sin código de V3. 5 de 6 agentes lo detectaron solos y pararon a pedir permiso en vez de forzar un `git checkout` (cero archivos tocados en el árbol real, verificado antes de relanzar). Relanzados sin aislamiento — la disciplina de cercado de archivos por prompt fue suficiente.
- **2026-08-10 — nota de proceso (por qué esta entrada dejó de crecer):** las rondas 1-3 de esta auditoría se registraron acá con el mismo nivel de detalle que ya vive en `backlog_auditoria_pantallas.md` (filas D-XX/C-XX con hallazgo, resolución y estado) — duplicación real señalada por Javier. Desde acá en adelante, los hallazgos de pantalla viven únicamente en el backlog; esta entrada se actualiza solo con snapshots de verificación y decisiones de proceso, hasta que el ciclo de auditoría cierre formalmente (momento en que se escribe el POC de cierre).
- **2026-08-10 — ronda 4, cambio de rigor (plan aprobado por Javier):** en vez de una sola pasada de auditoría, se abre un ciclo completo de "diamante" por cada hallazgo de diseño del backlog — auditoría (bruta→autocrítica→refinamiento) → auditoría axiomática (Suh, plantilla de `arnes/nucleo/mini_diamante_check_produccion.md`) → auditoría de ergonomía cognitiva (checklist nuevo, primera versión, sin precedente formalizado en el repo) → requisitos de código → ejecución. Aplica a D-09/D-10 (sin diamante todavía) **y** re-auditando D-01–D-08a (ya "ejecutados" con una sola pasada). Autoridad de aprobación: Claude autocertifica "listo" con evidencia mecánica; commit/push siguen siendo exclusivamente de Javier. Decisiones que ya bloqueaban el arranque (D-08b, D-09a/b, D-10b) se resolvieron con Javier antes de empezar — ver fila D-08/D-09/D-10 del backlog.
- **D-09 (Cotizador) cerrado bajo este nuevo proceso:** la auditoría axiomática, al verificar el código para poder explicarle a Javier las consecuencias de decidir D-09a, encontró un tercer hallazgo no reportado por Javier (D-09c: `items.eliminar()` es soft-delete pero ningún consumidor filtraba `anulado`, incluida la propuesta pública que ve el cliente) — confirma que el ciclo de auditoría profunda encuentra cosas que una sola pasada no encuentra, que era el objetivo del cambio de proceso. Detalle completo en la fila D-09 del backlog.

- **D-10 (Home real) cerrado bajo el mismo proceso:** la auditoría de computabilidad (verificar antes de diseñar, no después) encontró que el doc de contenido aprobado (`contenido_F01_home.md`) referencia 5 páginas que no existen todavía en el prototipo (F-09/F-10/F-11/F-12/F-18) y un token de diseño (`--color-bg-linen`) que nunca se implementó en `globals.css`. En vez de fabricar rutas o tokens que no existen, se resolvió con sustitutos reales verificados (WhatsApp para los CTA de contacto, `/portafolio` para los de "ver más", `bg-alt` como el linen real del sistema, `store.portafolio.publicados()` para los proyectos destacados en vez de los 3 "pendientes de selección" del doc) y se omitieron los CTA sin destino real en vez de dejar links rotos. Detalle completo en la fila D-10 del backlog.

- **D-01 (Web pública) re-auditado — la re-auditoría encontró una fuga de PII real, no cosmética:** `portafolio/[slug]/page.tsx` mostraba la dirección exacta del cliente (con número de apartamento) en una página pública sin autenticación, embebida además en JSON-LD indexable por buscadores. Corregido eliminando el campo, no sustituyéndolo (no existe un campo de "barrio" en el schema actual). Además: menú móvil del sitio público (`app-shell.tsx`) no tenía `onClick` — sin nav funcional en móvil, corregido con un dropdown real; se quitó jerga de proceso interno ("Diamante 4 · PoC 3") del footer público. Detalle en la fila D-01 del backlog.

- **D-02/D-03 (Nav) re-auditados — el hallazgo más sistémico de la ronda:** `<Button>` (`<button>`) anidado dentro de `<Link>` (`<a>`) en la navegación principal del ERP y del sitio público — inválido en HTML5, mismo defecto que C-05 (button en button) pero sin el crash de hidratación que hizo que C-05 se detectara en su momento. Un grep tras el fix del nav encontró el mismo patrón repetido en **9 archivos más** sin relación directa con D-02/D-03 (Compras, Equipo, Proyectos, Cotizador, Gates, Portafolio, Colecciones, portal cliente). En vez de parchear cada call site a mano, se agregó `LinkButton` a `components/veta/button.tsx` (un `<a>` con el look de `Button`, clases compartidas) y se reemplazó el patrón en los 11 archivos de una vez — evita que la próxima pantalla nueva reintroduzca el mismo bug por copiar el patrón viejo. Detalle en la fila D-02 del backlog.

- **D-04 (Compras) re-auditado:** `ItemOrdenCompra.recibidoCantidad`/`sinDefectos` quedaban en `0`/`false` para siempre — nada los actualizaba tras una recepción verificada 3/3. Corregido en la misma operación que ya mueve la OC a `recibida_verificada` (E-21), en `recepcionesMaterial.actualizarChecks`. Además, hallazgo sin resolver por decisión propia (no es un fix de código, es un dato de permisos): `ROLES_FLUJO_APROBACION` incluye un rol `'gerente'` que no existe en el sistema — queda como DP-01 en §3 del backlog para que Javier decida si el rol correcto es `compras`.

- **D-05 (Herramientas) re-auditado:** `herramientas.reponer()` creaba una `OrdenCompra` con `proveedorId: ''` cuando la herramienta no tenía proveedor asignado — el tipo declara ese campo no-nullable y el formulario manual de crear OC lo exige, así que era una vía silenciosa para saltarse esa regla. Corregido con guardia explícita + test nuevo.

- **D-06 (Finanzas) re-auditado:** el drill-down del dashboard mostraba `origen`/`estado` crudos (`en_pago`, `contrato_hito`) en vez de las etiquetas humanas que las 3 pantallas de detalle correspondientes ya usan para los mismos campos — corregido reusando el mismo texto. Sin hallazgos de datos/lógica.

- **D-07 (Taller) re-auditado — la implementación no coincidía con su propio diseño aprobado:** el doc pide explícitamente instalación/garantía "activa" contada "por estado", pero el código contaba cualquier registro sin mirar estado — un proyecto ya instalado o con garantía cerrada seguía inflando el conteo/filtro/badge para siempre. Corregido con dos sets de estados activos aplicados consistentemente en conteo, filtro, inclusión en lista y badges (el modal de detalle conserva el historial completo).

- **D-08a (Perfil Persona/Proveedor) re-auditado — cierre del batch:** mismo patrón de estados/orígenes crudos y montos con 2 decimales que D-06/D-08a's propia pantalla hermana, corregido en `equipo/[personaId]/page.tsx` y `compras/proveedores/[proveedorId]/page.tsx` (6 sitios). **D-08b (login interno) diseñado, no construido**, por decisión de Javier: `diseno_perfil_persona_proveedor_login_interno.md` — la verificación de computabilidad encontró que el mecanismo ya existe (portal cliente, `lib/auth/session.ts`, iron-session + email/documento sin hash, ya aprobado por el Supervisor) y solo hace falta extenderlo, no inventar uno nuevo. No se tocó `lib/auth/` ni se implementó nada ejecutable — el documento deja 3 preguntas explícitas para Javier antes de que alguien lo construya (cuándo, si autogestión permite editar, qué significa "login de proveedor" si el proveedor es una empresa).

**Cierre de la ronda 4 — los 10 diamantes del backlog (D-01 a D-10) están auditados con el ciclo completo (auditoría → axiomático → ergonomía → código) y verificados mecánicamente.** Hallazgos reales encontrados que la primera pasada no había detectado: fuga de PII de clientes en el portafolio público + JSON-LD indexable (D-01), nav con `<button>` anidado en `<a>` en 11 archivos incluida la navegación principal del ERP y del sitio público (D-02/D-03), ítems "eliminados" del Cotizador que seguían cobrándose en la propuesta pública que ve el cliente (D-09c), recepción de material que nunca marcaba los ítems como recibidos (D-04), una vía silenciosa para crear órdenes de compra sin proveedor real (D-05), y conteos de Instalación/Garantía en Taller que no implementaban lo que su propio diseño aprobado pedía (D-07). El resto de hallazgos fueron de consistencia (etiquetas crudas, formato de moneda) — reales pero de menor severidad. Una decisión de permisos quedó abierta sin resolver por decisión propia (DP-01, rol `compras` vs `gerente` en Compras).

### Verificación (ronda 4 — cierre completo D-01 a D-10, 2026-08-10)

`npx tsc --noEmit` = 0 · `npx eslint .` = 0 errores (archivos tocados, solo warnings preexistentes `@next/next/no-img-element`) · `npx next build` = 42/42 rutas, sin errores; `/` ahora renderiza el Home real (antes: redirect a `/landing`). `npx tsx lib/data/mock-store.test.ts` = 61/61. Nada commiteado — todo el batch (D-01 a D-10, más D-08b como documento de diseño) queda a la espera de la revisión en vivo de Javier.

**Próximo POC:** revisión en vivo de Javier sobre todo el batch; luego decidir DP-01 (rol de aprobación de Compras) y las 3 preguntas de D-08b si decide destrabar login interno.
