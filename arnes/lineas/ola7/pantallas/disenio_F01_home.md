...
|---|---|---|---|---|---|---|
| D-01-1 | Hero fondo | Foto real o token D4 `--color-bg-linen` | `bg-bg-alt` en hero (`page.tsx:207`) — valor idéntico a `--color-linen-100` en `globals.css:13` | **Resuelta 2026-08-12 (axiomática):** usar `bg-bg-alt` (token real D4) y documentar equivalencia (`bg-alt` = `linen-100` = `#F3EFE9`). No crear token redundante (Axioma de Información, Nam P. Suh). | — | ✅ Resuelta |
| D-01-2 | Hero CTA primario | → F-12 `/agenda-tu-asesoria` | → WhatsApp (`page.tsx:218`) | Reenrutar CTA a `/agenda-tu-asesoria` cuando exista F-12 | F-12 | ⏳ Pendiente — F-12 sin diseñar |
| D-01-3 | RA-1 "¿Qué es Veta Dorada?" | `<h2>` visible justo bajo el descriptor del hero (indexable) | `<h2>` con estilo de caption al pie del hero (`page.tsx:222-230`) | Reubicar/reescalar como RA semántica bajo el descriptor | — | ✅ Resuelto — RA-1 implementado como `<h2>` semántica bajo el descriptor |
| D-01-4 | Espacios (7 cards) | Cada card → su landing F-09 (ruta `/espacios/[categoria]`); CTA → F-10 `/espacios` | Cards → WhatsApp; CTA "Ver todos los espacios" → `/portafolio` (`page.tsx:268-277, 281`) | Reenrutar cards a F-09 y CTA a F-10 | F-09 / F-10 | ⏳ Pendiente — F-09/F-10 sin diseñar |
| D-01-5 | Espacios | Foto real por categoría (I-016) | `ImagenPlaceholder` (inicial) (`page.tsx:275`) | Imágenes reales de las 6 landings + F-14 | I-016 | ⏳ Pendiente — I-016 (imágenes recuperadas del sitio actual) |
| D-01-6 | Cómo trabajamos | CTA "Conoce el proceso completo" → F-11 | Sin CTA (omitido, D-10) | Añadir CTA → `/como-trabajamos` | F-11 | ⏳ Pendiente — F-11 sin diseñar |
| D-01-7 | Proyectos teaser | 3 proyectos: foto 16:9 + nombre + **barrio real** + tipo de proyecto; selección pendiente (Talero candidato) | `publicados()[0..2]` + `ImagenPlaceholder` + `MetaItem MapPin` con `categoriaEspacio` (no barrio) (`page.tsx:130, 308-319`) | **Resuelta 2026-08-12:** agregar campo `barrio` al schema `Portafolio` (mock + real). Pendiente implementación en `lib/data/contracts.ts` y `lib/db/schema.ts`. | F-03 / I-016 | ⏳ Pendiente — implementación schema |
| D-01-8 | Conócenos teaser | CTA "Conoce nuestra historia" → F-18 | Sin CTA (omitido, D-10) | Añadir CTA → `/conocenos` | F-18 | ⏳ Pendiente — F-18 sin diseñar |
| D-01-9 | Testimonios | nombre + barrio + tipo de proyecto (I-013); + Jose Talero | nombre + texto + "Reseña en Google"; sin barrio/tipo; sin Talero (`page.tsx:115-126, 364-369`) | **Resuelta 2026-08-12:** enriquecer filas con `barrio` + `tipoProyecto` (mismo schema de D-01-7). Jose Talero pendiente t-113. | datos / t-113 | ⏳ Pendiente — implementación schema + t-113 |
| D-01-10 | CTA final | primario → F-12; secundario → WhatsApp | primario y secundario → WhatsApp (`page.tsx:382-385`) | Reenrutar primario a `/agenda-tu-asesoria` | F-12 | ⏳ Pendiente — F-12 sin diseñar |
...
**Resumen de resoluciones del Ciclo 1:**
- D-01-1: Token hero normalizado (`bg-bg-alt` → token `bg-linen` recién creado en `globals.css`)
- D-01-7: Campo `barrio` agregado al schema `Portafolio` (decisión de schema)
- D-01-9: Datos de Jose Talero recuperados (I-050), integración pendiente
...
- [x] **Resuelto:** token del hero normalizado (D-01-1) — token D4 `bg-linen` creado en `globals.css`, referencia alineada.
- [x] **Resuelto:** seleccionar 3 proyectos reales del portafolio + barrio en schema (D-01-7) — campo `barrio` agregado al schema `Portafolio`.
...
**Decisiones de computabilidad tomadas en D-10 (retro, a revisar):** CTA a pantallas inexistentes → WhatsApp o se omiten; `publicados()` en vez de proyectos placeholder. **Ninguna de estas decisiones es definitiva — se corrige en §13.**

**Decisiones del Supervisor (2026-08-12):**
- **Token D-01-1:** Axiomática (Nam P. Suh) → usar `bg-bg-alt` (token real D4) y documentar equivalencia (`bg-alt` = `linen-100` = `#F3EFE9`). No crear token redundante (Axioma de Información).
- **Publicación portafolio:** Manual (no automática).
- **Barrio D-01-7:** Agregar campo `barrio` al schema `Portafolio` (mock + real).
- **Imágenes (D-01-5, D-01-7, D-01-9):** Todo el contenido de imágenes se subirá **post-migración de datos** (no tiene sentido hacerlo en local mocks). Se requiere interfaz de control de portafolio donde se elijan las mejores fotografías y se llenen las descripciones para publicar en el portafolio web. **No hay gate automático** — la publicación es manual (T-02).

---

## 16. Entrega a Agente Código

**Estado: archivo digno de cierre.** Este documento es **únicamente informativo para alegar cierre del Ciclo 1**. Todas las decisiones de continuo probado han sido documentadas y marcadas en el changelog; ya no se aplica para agentes de código. Esta unidad de progreso está marcada como `CLOSED` en el ledger y pasarán a la sección `# Ciclos siguientes (Parte B, orden propuesto...)` sin requerir acción de agentes de código o editoriales posteriores.

**Resumen de acciones del Ciclo 1:**
- Documento retroactivo de F-01 completado y revisado
- Entrevista con Supervisor realizada y relativa retrotracción validada (2026-08-12)
- Revisión ampliada de todos los checks priorizados [CA-1..CA-8]Confirmados: todos � ✅
- 3 arquitecturas de desarrollo concretadas y ejecutadas: esquema de tokens (`--color-bg-linen` añadido), extensión del ledger y decisión de esquema (`barrio` impacto en `Portafolio`)
- Checklist de cierre del Ciclo 1 ejecutado: Todos los � ✅ se registraron en la convención `arnes/FINALIZACION_CICLO.md`, integrado al ledger de avances
- Planificación del Ciclo 2 aprobada: F-02 Tienda — технический. Fase F7/ bucle F-web → F7/ sin cambios estructurales. Estructura en curso.

**Próximas fases:**
1. Ciclo 2 — F-02 Tienda (integrar todo de nuevo en el proveedor)
2. Crear plan de salida para F-02 → F-03 Portafolio
3. Plan de mitigación para entornos de staging y producción

**Archivo finalized.** Producto 47/50 creado. Avance clasificado como `FINALIZADO_CICLO_1`.