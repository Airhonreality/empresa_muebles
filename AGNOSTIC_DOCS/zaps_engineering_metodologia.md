...
📜 Estructura de Diseño para cada Zap (El Contrato)
Cada Zap en nuestro diseño debe especificarse bajo este contrato estricto antes de su codificación:

markdown
### `zap_nombre_script`
* **Trigger (Gatillo):** Bloque o componente que lo invoca (ej. Botón en Ficha de Proyecto).
* **Payload Entrada:** Qué campos mínimos espera en `payload.record`.
* **Consultas (Reads):** Qué namespaces lee vía `api.query()`.
* **Escrituras (Writes):** Qué registros guarda vía `api.saveItem()`.
* **Eventos (Dispatches):** Qué eventos de UI encola (`materia_sync`, `notify`, `download_file`, etc.).
