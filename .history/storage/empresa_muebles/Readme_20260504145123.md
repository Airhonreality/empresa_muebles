...
🌌 Mapa Macro: El Universo ERP (Axiomatic Sovereign)
He dividido el sistema en 4 Grandes Dominios. Cada esquema y componente debe vivir obligatoriamente en uno de estos:

Dominio	Propósito Teleológico	Entidades Clave (Schemas)	Prefijo Sugerido
CRM & Ventas	Captación e intención comercial.	leads, clients, proposals	crm_
Ingeniería & Cotización	Definición técnica y precio de la materia.	quote_request, quote_spaces, quote_items	quote_
Operaciones & Proyectos	La ejecución de la obra y la realidad.	projects, tasks, installations	proj_
Manufactura & Almacén	La transformación física de la madera.	production_orders, stock, suppliers	mfg_
🔍 Zoom al Módulo de Cotización (Redefinición de Nomenclatura)
Siguiendo tu visión, el "Cotizador" no es una entidad aislada, es un Proyector de Propuestas Técnicas. Vamos a renombrar los componentes para que su nombre describa exactamente su función dentro de la Cotización:

Nombre Actual (Entropía)	Nuevo Nombre Canónico	Razón Teleológica
vdo_identity_header	quote_header_identity	Identifica qué proyecto y cliente estamos cotizando.
vdo_quote_editor	quote_body_spaces	Es el cuerpo donde se edita la materia por ambientes.
vdo_economic_summary	quote_footer_settlement	"Settlement" (Liquidación) describe el cierre financiero de esta propuesta específica.
📜 El Plano Maestro de los Esquemas (La Evolución)
Para que el Cotizador esté perfectamente integrado con el resto del ERP, los esquemas deben evolucionar así:

Macro-Clase Entity: El "Cliente" y el "Proveedor" son proyecciones de una misma clase de entidad legal.
Macro-Clase Asset: El "Material" en inventario y el "Ítem de Catálogo" son proyecciones de la misma materia física.
La Cotización como "Borrador de Proyecto": Una cotización aceptada se convierte automáticamente en un Project y dispara una ProductionOrder.
🗺️ El Mapa de Componentes UI (Micro-Arquitectura)
Prefix Agnostic: Componentes universales (Input, Table, Guard).
Prefix Block: Unidades de negocio ruteables (Header, Editor, Settlement).
Prefix Atom: Elementos de estilo puro (Badges, Icons, Dividers).
