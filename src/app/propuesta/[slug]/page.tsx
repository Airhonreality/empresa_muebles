...
import { resolvePublicLinkBySlug, type PublicField } from '@/server/public-links';
...
/** Public, isolated document route. It never mounts the authenticated app shell. */
export default async function PublicProposalPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const document = await resolvePublicLinkBySlug(slug);
  if (!document) notFound();

  return (
    <main className="min-h-screen bg-background px-6 py-12 text-foreground">
      <article className="mx-auto max-w-3xl rounded-xl border bg-card p-8 shadow-sm">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Propuesta</p>
