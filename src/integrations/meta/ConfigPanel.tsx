...
export function ConfigPanel({ envPresence, onSave }: IntegrationConfigPanelProps) {
  ...
  const allFilled = useMemo(() => FIELD_KEYS.every(key => values[key]?.trim()), [values]);
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold">Meta Messaging</h3>
        <p className="text-xs text-muted-foreground">
          Configura el token de pagina, el usuario de Instagram y las claves de webhook.
        </p>
      </div>
